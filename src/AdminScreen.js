...
const AdminScreen = () => {
  ...
  async function fetchArrivals() {
    try {
      const response = await fetch('https://az-medical.onrender.com/api/allArrivals');
  ...
  function generateArrivalsPerHourData(arrivalsPerHourData) {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      formattedArrivalsPerHourData.unshift({ hour: formattedHour, arrivals: arrivals });
    }
  ...
  async function updateArrivalsPerHourData() {
    ...
    const formattedArrivalsPerHourData = generateArrivalsPerHourData(arrivalsPerHourData);
  ...
  async function updateBarChartData() {
    try {
      // Fetch arrivals data
      const responseArrivals = await fetch('https://az-medical.onrender.com/api/allArrivals');
      const arrivals = await responseArrivals.json();
  
      console.log(arrivals)
      // Fetch doctors data
      const responseDoctors = await fetch('https://az-medical.onrender.com/api/doctors');
  ...
  console.log(doctors)
      ...
      const filteredArrivals = arrivals.filter(arrival => new Date(arrival.arrivalTime) > currentDate);
...
filteredArrivals.forEach(arrival => {
  ...
  arrivalsPerDoctor[doctorName] = (arrivalsPerDoctor[doctorName] || 0) + 1;
});

// Convert the object into an array of objects with doctor names and patient counts
const barChartData = Object.entries(arrivalsPerDoctor).map(([doctor, patients]) => ({
  doctor: doctor,
  patients: patients
}));
...
setBarChartData(barChartData);
  ...
  const comingSoonData = [
    { id: 1, name: "Dr. Smith", checkInTime: "08:00 AM", checkOutTime: "04:00 PM" },
    { id: 2, name: "Dr. Johnson", checkInTime: "09:00 AM", checkOutTime: "05:00 PM" },
    { id: 3, name: "Dr. Williams", checkInTime: "08:30 AM", checkOutTime: "04:30 PM" },
    { id: 4, name: "Dr. Brown", checkInTime: "08:15 AM", checkOutTime: "04:15 PM" },
    { id: 5, name: "Dr. Miller", checkInTime: "08:45 AM", checkOutTime: "04:45 PM" },
    // Add more data for other doctors
  ];
            ...
            <Box sx={{ width: "45%", marginBottom: 4 }}>
              ...
              <ResponsiveContainer width="100%" height={250}>
                <LineChart data={arrivalsPerHourData}>
                  ...
                  <XAxis dataKey="hour"  begin={true} fontSize={14} fontWeight="bold"/>
                  <YAxis fontSize={14} fontWeight="bold"/>
                  <Tooltip />
                  <Line type="monotone" dataKey="arrivals" stroke="#8884d8" strokeWidth={3} animationDuration={1500} animationEasing="ease" />
                </LineChart>
              </ResponsiveContainer>
            </Box>
          ...
          <Box sx={{ width: "45%", marginBottom: 4 }}>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={barChartData}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="doctor"  textAnchor="end" interval={0} fontSize={10} fontWeight="bold" tick={<CustomXAxisTick />} />
                <YAxis />
                <Tooltip />
    ...
    <PieChart>
      ...
      <Tooltip />
          ...
          <Box sx={{ width: "45%", marginBottom: 4 }}>
            ...
            <Box sx={{ border: "1px solid #ccc", borderRadius: "5px", padding: 2 }}>
              {selectedDoctor ? (
                comingSoonData
                  .filter((data) => data.id === selectedDoctor)
                  .map((doctorData) => (
                    <div key={doctorData.id}>
                      <Typography variant="subtitle1">{doctorData.name}</Typography>
